...
const int timeThreshold = 110;
int counter1 = 0;
int counter2 = 0;
int buttonState1 = LOW;
int buttonState2 = LOW;
long previousMillisButton1 = 0;
long previousMillisButton2 = 0;
int delayButton = 50;

/* Draw Delay */
long previousMillisDraw = 0;
int delayDraw = 50;             //Millis
...
void CompassReading(){
  ...
  float heading = compass.heading((LSM303::vector<int>){ 
    1, 0, 0        }
  ); //Vector reference for noth heading {X, Y, Z}
  headingInt = heading;
  if (headingInt < 1) {
...
int ButtonPress2() {
  ...
  for(i = 0; i < 500; i++){
    rawValue = rawValue + analogRead(A8);
  }
...
void setup(void) {
  ...
  config.pinMode(12, CMP, LOW, 0.61875);//pin, mode, type, threshold(v)
}
...
void loop(void) {
  ...
  /* picture loop */
  unsigned long currentMillisDraw = millis();
  if (currentMillisDraw - previousMillisDraw >= delayDraw) { //Eliminates Delay
    previousMillisDraw = currentMillisDraw;
    u8g.firstPage();                                //Draw code
    do {
      draw();
    } 
    while( u8g.nextPage() );
  ...
  if (buttonState2 == HIGH) {
    ...
    /* Runs Start up after waking from sleep */
    buttonState2 = LOW;
